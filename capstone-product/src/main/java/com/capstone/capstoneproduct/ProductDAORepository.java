...
public class ProductDAORepository implements ProductDAO {
	...
	public boolean postProduct(Product product) throws SQLException {
		...
		int i = jt.update(query, new Object[] {product.getP_id(), product.getP_name(), product.getImage_id(), product.getP_price(), product.getP_description()});
		if(i>0)
			return true;
		else
			return false;
	...
	public boolean updateProduct(Product product, int id) throws SQLException {
		...
		if(ps.execute())
		{
			query = "update product set p_name = ?, image_id = ?, p_price = ?, p_description = ? where p_id = " + id;
			ps = con.prepareStatement(query);
			ps.setString(1, product.getP_name());
			ps.setString(2, product.getImage_id());
			ps.setDouble(3, product.getP_price());
			ps.setString(4, product.getP_description());
	...
	public Product getProduct(int id) throws SQLException {
		...
		while(rs.next())
		{
			...
			p.setImage_id(rs.getString(3));
			p.setP_price(rs.getDouble(4));
			p.setP_description(rs.getString(5));
		}
		return p;
	}
